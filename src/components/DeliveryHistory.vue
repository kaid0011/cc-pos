  ...
  <div class="logistics-management">
    ...
    <div class="row-col-table">
      ...
      <div v-else-if="allTransactions.length === 0" class="text-center text-grey q-pa-lg text-h6">
        No history found for this delivery.
      </div>
      ...
      >
        <!-- All from logistics_delivery_history -->
        <div class="col bordered">
          <div><span class="text-weight-bold">Date: </span>{{ formatDate(transaction.delivery_date) || "[NOT SET]" }}</div>
          <div><span class="text-weight-bold">Time: </span>{{ transaction.delivery_time ?? "-" }}</div>
...
const props = defineProps({
  logisticsDeliveryId: { type: [String, Number], required: false },
});
...
const effectiveId = computed(() => {
  const n = Number(props.logisticsDeliveryId);
...
async function loadHistory() {
  ...
  loading.value = true;
  try {
    const rows = await transactionStore.fetchDeliveryHistoryByDeliveryId(effectiveId.value);
    allTransactions.value = Array.isArray(rows) ? rows : [];
  } catch (e) {
    console.error("Error initializing delivery history:", e);
    allTransactions.value = [];
  } finally {
    loading.value = false;
  }
