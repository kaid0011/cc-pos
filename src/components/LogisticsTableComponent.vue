              ...
              >
                ...
                <div class="q-pa-sm">
                  ...
                  <div>
                    ...
                    <span
                      :class="{
                        'mark-yellow text-weight-bolder':
                          String(
                            fallbackDeliveryBits(logistics).driver_id ?? ''
                          ) === String(activeDriverTab),
                      }"
                    >
                      <NotSetText
                  ...
                  <div>
                    ...
                    <NotSetText
                      :value="
                        formatAddress(fallbackDeliveryBits(logistics).address)
                      "
                    />
                  </div>
                </div>
...
const props = defineProps({
  ...
  deliveryExceptions: { type: Array, default: () => [] }, // ← add
});
...
const NotSetText = defineComponent({
  ...
  setup(props) {
    return () => {
      ...
      if (str) return h("span", str); // Why: only show fallback if empty
      return h(
        "span",
...
const ordersOrPlaceholder = (logistics) => {
  ...
  const exc = getDeliveryExceptionsFor(logistics).map((ex) => ({
    kind: "exception",
    order: logistics?.order || null, // use the single order if present; else null => "Create Order"
    ex,
  }));
...
const displayUnpaid = computed(() =>
  Math.max(displayTotal.value - displayPaid.value, 0)
);
...
async function addPayment() {
  ...
  if (!selectedPaymentType.value || paymentAmount.value <= 0) {
    $q.notify({
      type: "negative",
      message: "Select a type and enter a valid amount.",
    });
    return;
  }
  if (
...
// Optional batched fetchers if your store exposes them
const fetchProfilesByIds = async (ids = []) => {
  if (!ids.length) return [];
  if (typeof transactionStore.fetchProfilesByIds === "function") {
    return await transactionStore.fetchProfilesByIds(ids); // expect [{id, name, ...}]
  }
  // fallback: fetch one-by-one if available
  ...
  return []; // final fallback (no API available)
};
...
const resolveDriverProfiles = async (collectionDriverId, deliveryDriverId) => {
  ...
  // 1) try local cache (driverOptions)
  const local = new Map();
  ...
  // 2) fetch only missing from `profiles`
...
const createOrder = async (logistics) => {
  ...
  // determine pricing group id from customer
  ...
  if (!pricingGroupId) {
    // Why: group is required to fetch correct price list
    $q.notify({
      type: "warning",
  ...
  // normalize first collection / delivery
  const collection =
  ...
  if (collection) {
    ...
    // ✅ set to profile object if found, else keep a friendly fallback (string or null)
    transactionStore.selectedCollectionDriver = collectionDriverProfile
      ? collectionDriverProfile // { id, name, ... } from profiles
  ...
  if (delivery) {
    ...
    // ✅ set to profile object if found, else keep a friendly fallback
    transactionStore.selectedDeliveryDriver = deliveryDriverProfile
      ? deliveryDriverProfile // { id, name, ... } from profiles
...
// observe the computed map
watch(exceptionsByDeliveryId, (m) => {
  const keys = [...m.keys()];
});

function getDeliveryExceptionsFor(logistics) {
  const key = String(logistics?.logistics_id ?? logistics?.id ?? "");
  const out = key ? exceptionsByLogisticsId.value.get(key) || [] : [];
  return out;
}
...
// observe
watch(exceptionsByLogisticsId, (m) => {
  const keys = [...m.keys()];
});

function fallbackDeliveryBits(logistics) {
  const d = logistics?.deliveries?.[0] || {};
  const contact = d?.contact_person || d?.customer_contact_persons || {};
  return {
