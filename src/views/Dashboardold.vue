        ...
        <div class="list-area">
          ...
          >
            <template v-slot:body-cell-name="props">
              <q-td :props="props" class="name-column">
                <div class="text-p text-left wrap-text">
                  ...
                  <span v-if="props.row.unit === 'kg' || props.row.unit === 'sqft'">
            ...
            <template v-slot:body-cell-unit_price="props">
              <q-td :props="props" class="unit-price-column">
  ...
  <q-card>
    ...
    <q-dialog v-model="showMeasurementDialog" persistent>
      <q-card>
        ...
        <q-card-section>
                    <!-- Dynamically display radio buttons for services with non-null prices -->
                    <q-option-group
          ...
          <q-input v-model="length" label="Length (in ft)" type="number" />
          <q-input v-model="breadth" label="Breadth (in ft)" type="number" />
    
          <!-- Display the total sqft -->
          <div class="text-h6">
            Total: {{ totalSqft.toFixed(2) }} sqft
          </div>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn @click="addToTransaction" label="Add" color="primary" />
...
const openDialog = (item) => {
  ...
  // Check if the item has unit 'kg' to open the weight dialog
  if (selectedItem.value.unit === "kg") {
      showWeightDialog.value = true; // Open weight dialog for items with unit 'kg' and valid services
  } else if (selectedItem.value.unit === "sqft") {
      showMeasurementDialog.value = true;  // Open the measurement dialog for sqft with valid services
  } else {
    // Open the default dialog for quantity input for other units
...
const selectedProcess = ref('Laundry'); // Set default to 'Laundry'

const length = ref(1);
const breadth = ref(1);
const totalSqft = computed(() => length.value * breadth.value);
...
const availableServices = computed(() => {
  ...
  if (selectedItem.value.laundry_price !== null) {
    services.push({ label: `Laundry - $${selectedItem.value.laundry_price.toFixed(2)} /kg`, value: 'Laundry' });
  }
  if (selectedItem.value.dryclean_price !== null) {
    services.push({ label: `Dry Clean - $${selectedItem.value.dryclean_price.toFixed(2)} /kg`, value: 'DryClean' });
  }
  if (selectedItem.value.pressing_price !== null) {
    services.push({ label: `Pressing Only - $${selectedItem.value.pressing_price.toFixed(2)} /kg`, value: 'PressingOnly' });
  }
  if (selectedItem.value.others_price !== null) {
    services.push({ label: `Others - $${selectedItem.value.others_price.toFixed(2)} /kg`, value: 'Others' });
  }
...
const addToTransaction = () => {
  ...
  for (const [key, suffix] of Object.entries(services)) {
    if (quantities.value[key] > 0 || (key === selectedProcess.value && (totalSqft.value > 0 || totalKg.value > 0))) {
      const name = selectedItem.value.name;  // Keep the item name clean without suffix
      const unit = selectedItem.value.unit;
      const existingItem = transactionItems.value.find(
        (i) => i.name === name && i.suffix === suffix
      );

      // Determine the unit price based on the selected service (suffix)
      let unit_price = 0;
      switch (selectedProcess.value) {
        case "Laundry":
          unit_price = selectedItem.value.laundry_price;
          break;
        case "DryClean":
          unit_price = selectedItem.value.dryclean_price;
          break;
        case "PressingOnly":
          unit_price = selectedItem.value.pressing_price;
          break;
        case "Others":
          unit_price = selectedItem.value.others_price;
          break;
      }

      // Calculate the total price based on unit type (weight for kg, sqft for area)
      let price = 0;
      if (unit === "kg") {
        price = unit_price * totalKg.value;  // For kg, multiply by the total weight
      } else if (unit === "sqft") {
        price = unit_price * totalSqft.value;  // For sqft, multiply by total area
      } else {
        price = unit_price * quantities.value[key];  // For other units, multiply by quantity
      }

      if (existingItem) {
        existingItem.quantity += quantities.value[key] || 1;
        existingItem.unit_price = unit_price.toFixed(2);
        existingItem.price = price.toFixed(2);
        existingItem.size = unit === "kg" ? totalKg.value : totalSqft.value;
      } else {
        ...
        transactionItems.value.push({
          ...
          name: name,
          suffix: suffix,
          quantity: quantities.value[key] || 1,
          unit_price: unit_price.toFixed(2),
          price: price.toFixed(2),
          unit: unit,
          size: unit === "kg" ? totalKg.value : totalSqft.value  // Store the weight (kg) or area (sqft)
        });
      }
    }
  }

  // Close the dialog after adding the item to the transaction
