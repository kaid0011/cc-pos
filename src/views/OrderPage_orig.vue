    ...
    <div class="col-6 preview-right-container">
      ...
      <q-card flat class="preview-card q-mt-md">
        <div class="row">
          ...
          <div class="col text-summary">
    
            <div>{{ order?.job_type }}</div>
            <div class="text-uppercase">{{ order?.goods_status }}</div>
            <div class="text-uppercase">{{ order?.logistics_status }}</div>
            <div class="text-uppercase">{{ order?.payment_status }}</div>
            <div>{{ customer?.payment_type }}</div>
            <div>{{ order?.no_packets_hangers }}</div>
          </div>
          </div>
      </q-card>
      ...
      <q-card flat class="preview-card q-mt-md">
        ...
        <div v-if="instructionsOnetime.length > 0 || instructionsRecurring.length > 0">
          <div v-for="instruction in instructionsRecurring" :key="instruction.id" class="per-instruction q-mb-sm">
            <div class="text-p">
              <span>
                <q-icon name="circle" color="primary" size="8px" class="q-mr-sm q-ml-md" />
              </span>
              {{ instruction.description }}
            </div>
            <div class="instruction-chips q-ml-lg">
              <q-chip square color="purple-10" text-color="white" class="instructions-type">Recurring</q-chip>
              <span> | </span>
              <q-chip
          ...
          <div v-for="instruction in instructionsOnetime" :key="instruction.id" class="per-instruction q-mb-sm">
            <div class="text-p">
              <span>
                <q-icon name="circle" color="primary" size="8px" class="q-mr-sm q-ml-md" />
              </span>
            ...
            <div class="instruction-chips q-ml-lg">
              <q-chip square color="pink-10" text-color="white" class="instructions-type">One-time</q-chip>
              <span> | </span>
              <q-chip
                v-for="section in instruction.to"
                :key="section"
        ...
        <div v-else class="text-center text-grey">No instructions listed.</div>
      </q-card>

      <!-- Error Reporting Section -->
      <q-card flat class="preview-card q-mt-md">
        <div class="text-h6 text-center text-weight-bold">Error Reporting</div>
        <q-separator class="q-my-sm" />
        <div v-if="reports.length > 0">
          <div v-for="report in reports" :key="report.id" class="per-report">
            <div class="text-p">
              <span>
                <q-icon name="circle" color="primary" size="8px" class="q-mr-sm q-ml-md" />
              </span>
        ...
        <div v-else class="text-center text-grey">No error reports listed.</div>
      </q-card>
    </div>
  </div>
</template>
