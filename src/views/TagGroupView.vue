<template>
  <div class="tags-view-container">
    <div class="row justify-center q-col-gutter-md q-pa-md">
      <div class="col-auto">
        <q-btn color="primary" @click="downloadTagPDF">Download Tag Slip PDF</q-btn>
      </div>
      <div class="col-auto">
        ...
        <q-card flat class="tag-card" ref="printableCard">
          <div class="row tag-card-header justify-between items-center">
            ...
            <div class="col text-h6 q-ma-md">
              <div>Order No: <span class="text-summary text-h5">{{ logistics.order?.order_no || 'N/A' }}</span></div>
              <div>Order Date: <span class="text-summary text-h5">{{ new Date(logistics.order?.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) }}</span></div>
            </div>
          ...
          <div class="row justify-between q-pa-md">
            <div class="col text-subtitle1">
              <div>Customer Name: <span class="text-summary">{{ logistics.customer?.name || 'N/A' }}</span></div>
              <div>
                Contact Nos: <span class="text-summary">{{ logistics.customer?.contact_no1 || 'N/A' }}<span v-if="logistics.customer?.contact_no2"> / {{ logistics.customer.contact_no2 }}</span></span>
              </div>
              ...
              <q-card flat style="border: solid; border-width: 1px; border-radius: 0">
                <q-card flat class="tags">
                  <div class="text-center row q-pa-xs">
                    <div class="col"><div>{{ logistics.tagCategoryCounts.clothing || 0 }}</div><div>A</div></div>
                    <div class="col"><div>{{ logistics.tagCategoryCounts.bedding || 0 }}</div><div>B</div></div>
                    <div class="col"><div>{{ logistics.tagCategoryCounts.curtain || 0 }}</div><div>C</div></div>
                    <div class="col"><div>{{ logistics.tagCategoryCounts.sofa || 0 }}</div><div>D</div></div>
                    <div class="col"><div>{{ logistics.tagCategoryCounts.stuffed_toy || 0 }}</div><div>E</div></div>
                    <div class="col"><div>{{ logistics.tagCategoryCounts.carpet || 0 }}</div><div>F</div></div>
                    <div class="col q-mr-sm"><div>{{ logistics.tagCategoryCounts.others || 0 }}</div><div>G</div></div>
                    <div class="col-6 tag-details">{{ logistics.formattedTagDetails }}</div>
                  </div>
                </q-card>
              </q-card>
            </div>
          ...
          <div class="row tag-card-header text-subtitle1">
            <div class="col col-3 text-weight-bold tag-card-cell text-center">Item</div>
            <div class="col col-2 text-weight-bold tag-card-cell text-center">Method</div>
            <div class="col col-2 text-weight-bold tag-card-cell text-center">Price</div>
            <div class="col col-1 text-weight-bold tag-card-cell text-center">Pcs</div>
            <div class="col col-1 text-weight-bold tag-card-cell text-center">Qty</div>
            <div class="col col-3 text-weight-bold tag-card-cell text-center">Remarks</div>
          </div>

          <div v-if="logistics.order_transactions?.length > 0" class="text-subtitle1">
            <div v-for="item in logistics.order_transactions" :key="item.item_name" class="row">
              <div class="col col-3 tag-card-cell">{{ item.item_name }}</div>
              <div class="col col-2 tag-card-cell text-center">{{ item.process }}</div>
              <div class="col col-2 tag-card-cell text-center">{{ `$${item.price.toFixed(2)}` }}</div>
              <div class="col col-1 tag-card-cell text-center">{{ item.pieces }}</div>
              <div class="col col-1 tag-card-cell text-center">{{ item.quantity }}</div>
              <div class="col col-3 tag-card-cell"></div>
            </div>
          </div>

          <div v-else class="text-center text-grey q-my-md text-subtitle1">No items added to the list.</div>

          <div class="row tag-card-footer q-py-sm text-subtitle1">
            <div class="col col-5 text-weight-bold text-uppercase"></div>
            <div class="col col-2 text-weight-bold text-uppercase text-right q-pr-md">Total :</div>
            <div class="col col-1 text-weight-bold text-center">{{ logistics.totalPcs }}</div>
            <div class="col col-1 text-weight-bold text-center">{{ logistics.totalQty }}</div>
          ...
          <div class="row text-center q-col-gutter-x-sm text-subtitle1">
            <div class="col"><div class="tag-borders">{{ logistics.tagCategoryCounts.clothing || 0 }}</div></div>
            <div class="col"><div class="tag-borders">{{ logistics.tagCategoryCounts.bedding || 0 }}</div></div>
            <div class="col"><div class="tag-borders">{{ logistics.tagCategoryCounts.curtain || 0 }}</div></div>
            <div class="col"><div class="tag-borders">{{ logistics.tagCategoryCounts.sofa || 0 }}</div></div>
            <div class="col"><div class="tag-borders">{{ logistics.tagCategoryCounts.stuffed_toy || 0 }}</div></div>
            <div class="col"><div class="tag-borders">{{ logistics.tagCategoryCounts.carpet || 0 }}</div></div>
            <div class="col"><div class="tag-borders">{{ logistics.tagCategoryCounts.others || 0 }}</div></div>
          </div>
        </q-card>
      </q-card>
    </div>
...
const DRIVER_NAMES = {
  Ass: 'Ass', Ken: 'Ken', Ang: 'Ang', Pea: 'Pea', You: 'You',
  Sfc: 'Self-collect', Nsc: 'Ng Soo Chong', Cha: 'Chia',
  Des: 'Des', Cue: 'Cue', Are: 'Are', Tin: 'Ting'
};
  ...
  try {
    ...
    matchedOrders.value = fullDetails.map((orderDetails) => {
      ...
      const formattedTagDetails = (() => {
        const collectionDate = formatDate(orderDetails.collection?.[0]?.collection_date);
        const deliveryDate = formatDate(orderDetails.delivery?.[0]?.delivery_date);
...
const PrintTag = () => {
  ...
  tags.forEach(tag => {
    const count = Array.from(tag.querySelectorAll(".col")).reduce((sum, col) => {
  ...
  html2pdf().set(options).from(container).toPdf().output("blob").then((blob) => {
    const pdfUrl = URL.createObjectURL(blob);
    const newWindow = window.open(pdfUrl, "_blank");
    if (newWindow) {
      setTimeout(() => {
